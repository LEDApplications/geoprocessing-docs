...
. remove all standalone line segments (look at the attribute table) and then run "unsplit lines"
...
. merge the coastal unsplit lines and the continental bounary lines
. run feature to polygon
. delete the state of AK, the CONUS, and that little area in seattle.
. finally clip out the water.coastline_polygons (islands) from the coastline mask
